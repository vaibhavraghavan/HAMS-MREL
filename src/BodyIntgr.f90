...
!   --------------------------------------------------------
      ...
      RKN(:)=RKBN(IEL,JEL,IS,:)                                      ! Rankine 1/r term
      GRN(:)=CGRN(IEL,JEL,IS,:)                                      ! Combination of the Local and Oscillatory term of the Green's function
...
!  Field point changes, source element keeps the same as in the first quadrant
      ...
      IF (FLAG.EQ.1) THEN
        DUM=RKBN(IEL,JEL,IS,1)+CGRN(IEL,JEL,IS,1)*DS(JEL)          ! The sum of the rankine term and the wave term of the Green's function
      ELSE
