...
MODULE SingularIntgrMulti
   ...
   PUBLIC :: SGLINTBD_MULTI_TRI
   PUBLIC :: SGLINTBD_MULTI_QUAD
   PUBLIC :: SGLINTBD_MULTI_TRI2
   PUBLIC :: SGLINTBD_MULTI_QUAD2
...
!------------------------------------------------------------------------------- 
       ...
       SUBROUTINE SGLINTBD_MULTI_TRI(IS,IEL,JEL,SIJ,DIJ,IRR)                                 ! This is for triangular elements
       IMPLICIT   NONE
       ...
       IF (IRR.EQ.1) THEN
        ...
        IF (ISX.EQ.1.AND.ISY.EQ.0) THEN                                                    ! When i is used: iNCN, iXYZ, it seems to be for the irregular frequencies. Check this? Check ReadPanelMesh for this
         XF(1)=SY_MULTI(IS,1)*XYZ_GLOBAL_MULTI_COMB_P(IEL,1)
         XF(2)=SX_MULTI(IS,1)*XYZ_GLOBAL_MULTI_COMB_P(IEL,2)
         XF(3)=          XYZ_GLOBAL_MULTI_COMB_P(IEL,3) 
        ELSE
         XF(1)=SX_MULTI(IS,1)*XYZ_GLOBAL_MULTI_COMB_P(IEL,1)
         XF(2)=SY_MULTI(IS,1)*XYZ_GLOBAL_MULTI_COMB_P(IEL,2)
         XF(3)=         XYZ_GLOBAL_MULTI_COMB_P(IEL,3) 
        ENDIF

        XQ(1,:)=XYZ_GLOBAL_MULTI_COMB(NCON_MULTI_COMB(JEL,1),:)
        XQ(2,:)=XYZ_GLOBAL_MULTI_COMB(NCON_MULTI_COMB(JEL,2),:)
        XQ(3,:)=XYZ_GLOBAL_MULTI_COMB(NCON_MULTI_COMB(JEL,3),:)
        IF (NCN_MULTI_COMB(JEL).EQ.4) XQ(4,:)=XYZ_GLOBAL_MULTI_COMB(NCON_MULTI_COMB(JEL,4),:)
        
       ELSEIF (IRR.EQ.2) THEN                                                     ! With IRR = 2,3 and 4, it is for irregular frequency removal
        ...
        XQ(1,:)=iXYZ_GLOBAL_MULTI_COMB(iNCON_MULTI_COMB(JEL,1),:)
        XQ(2,:)=iXYZ_GLOBAL_MULTI_COMB(iNCON_MULTI_COMB(JEL,2),:)
        XQ(3,:)=iXYZ_GLOBAL_MULTI_COMB(iNCON_MULTI_COMB(JEL,3),:)
        IF (iNCN_MULTI_COMB(JEL).EQ.4) XQ(4,:)=iXYZ_GLOBAL_MULTI_COMB(iNCON_MULTI_COMB(JEL,4),:)   
        
       ELSEIF (IRR.EQ.3) THEN
           
        IF (ISX.EQ.1.AND.ISY.EQ.0) THEN
         XF(1)=SY_MULTI(IS,1)*iXYZ_GLOBAL_MULTI_COMB_P(IEL,1)
         XF(2)=SX_MULTI(IS,1)*iXYZ_GLOBAL_MULTI_COMB_P(IEL,2)
         XF(3)=          iXYZ_GLOBAL_MULTI_COMB_P(IEL,3) 
        ELSE
         XF(1)=SX_MULTI(IS,1)*iXYZ_GLOBAL_MULTI_COMB_P(IEL,1)
         XF(2)=SY_MULTI(IS,1)*iXYZ_GLOBAL_MULTI_COMB_P(IEL,2)
         XF(3)=         iXYZ_GLOBAL_MULTI_COMB_P(IEL,3) 
        ENDIF
...
100    CONTINUE
       ...
       IF (IRR.EQ.1.OR.IRR.EQ.3) THEN                              ! The part with IRR=3 has not been modified yet. This is for irregular frequencies
           
       IF (NCN_MULTI_COMB(JEL).EQ.3) THEN
       GOTO 80     
       ELSEIF (NCN_MULTI_COMB(JEL).EQ.4.AND.ITNO.EQ.2) THEN
       GOTO 80            
       ELSEIF (NCN_MULTI_COMB(JEL).EQ.4.AND.ITNO.EQ.1) THEN      
       ITNO=2
       XV(1,:)=XQ(1,:)  ! point D
       XV(2,:)=XQ(3,:)  ! point A
       XV(3,:)=XQ(4,:)  ! point B
       ...
       IF (iNCN_MULTI_COMB(JEL).EQ.3) THEN                                     ! This part is related to irregular frequencies
       GOTO 80     
       ELSEIF (iNCN_MULTI_COMB(JEL).EQ.4.AND.ITNO.EQ.2) THEN
       GOTO 80            
       ELSEIF (iNCN_MULTI_COMB(JEL).EQ.4.AND.ITNO.EQ.1) THEN      
       ITNO=2
       XV(1,:)=XQ(1,:)  ! point D
       XV(2,:)=XQ(3,:)  ! point A
       XV(3,:)=XQ(4,:)  ! point B
...
80    CONTINUE
      ...
      RETURN
      END SUBROUTINE SGLINTBD_MULTI_TRI
...
!------------------------------------------------------------------------------- 
       ...
       SUBROUTINE SGLINTBD_MULTI_QUAD(IS,IEL,JEL,SIJ,DIJ,IRR)                                            ! This is for quardilateral elements    
       IMPLICIT   NONE
       ...
       ELSEIF (IRR.EQ.2) THEN                            ! IRR = 2,3 and 4 are related to Irregular frequency removal
...
!      Judge if the quadrilateral element is a flat panel
       PL(1,:)=XV(2,:)-XV(1,:)                                           ! There is no check here for the flatness?
       PL(2,:)=XV(3,:)-XV(1,:)
       PL(3,:)=XV(4,:)-XV(1,:)

!      Calculate singularity

        PO(:)=(XV(1,:)+XV(2,:)+XV(3,:)+XV(4,:))/4.D0                     ! This will calculate the mid point of the panel

        POV(1,:)=XV(1,:)-PO(:)   !PO1                                    ! Getting the corresponding vector going from the mid point to the edge node
        POV(2,:)=XV(2,:)-PO(:)   !PO2                                    ! Getting the corresponding vector going from the mid point to the edge node
        
        UN(1)=POV(1,2)*POV(2,3)-POV(1,3)*POV(2,2)    !  PO1 X PO2        ! This is the cross-product of the two vector which should give the normal to the panel
        UN(2)=POV(1,3)*POV(2,1)-POV(1,1)*POV(2,3)
        UN(3)=POV(1,1)*POV(2,2)-POV(1,2)*POV(2,1)
        
        UNITI(:)=POV(1,:)/SQRT(POV(1,1)**2+POV(1,2)**2+POV(1,3)**2)       ! Unit vector connecting the mid point of the panel to the first point
        UNITK(:)=UN(:)/SQRT(UN(1)**2+UN(2)**2+UN(3)**2)                  ! Unit normal vector to the plane of the panel
        UNITJ(1)=UNITK(2)*UNITI(3)-UNITK(3)*UNITI(2)                     ! Finding the unit cross product vector of UNITI and UNITK                    
        UNITJ(2)=UNITK(3)*UNITI(1)-UNITK(1)*UNITI(3)
        UNITJ(3)=UNITK(1)*UNITI(2)-UNITK(2)*UNITI(1)

!        XF(:)=XV(3,:)
        POF(:)=XF(:)-PO(:)   ! Field point in local coordinates
        X=UNITI(1)*POF(1)+UNITI(2)*POF(2)+UNITI(3)*POF(3)               ! This is a dot product. This gives the extent to which the UNITI, UNITJ and UNITK vectors are directed along POF.
        Y=UNITJ(1)*POF(1)+UNITJ(2)*POF(2)+UNITJ(3)*POF(3)
        Z=UNITK(1)*POF(1)+UNITK(2)*POF(2)+UNITK(3)*POF(3)
        
        POV(3,:)=XV(3,:)-PO(:)  !POD                                    ! Similar to POV(1,:) and POV(2,:)
        POV(4,:)=XV(4,:)-PO(:)  !POA
        
        DO I=1,4
          DO J=1,3
             PON(I,J)=POV(I,J)-(UNITK(1)*POV(I,1)+UNITK(2)*POV(I,2)+UNITK(3)*POV(I,3))*UNITK(J)     ! What does this give? If the panel is along one plane, PON would be the same as POV. Since POV and UNITK will always be perpendicular. UNITK is derived from the cross product of PO1 and PO2.
          ENDDO
        ENDDO                    
...
80    CONTINUE
      ...
      RETURN
       END SUBROUTINE SGLINTBD_MULTI_QUAD
...
!------------------------------------------------------------------------------- 
       ...
       SUBROUTINE SGLINTBD_MULTI_TRI2(XF,JEL,RK,IRR)                                                     ! This is for triangular elements
       IMPLICIT   NONE
       ...
       ELSEIF (IRR.EQ.2) THEN                        ! IRR = 2,3,4 are when removing irregular frequencies
...
100    CONTINUE
       ...
       IF (iNCN_MULTI_COMB(JEL).EQ.3) THEN
       GOTO 80
       ELSEIF (iNCN_MULTI_COMB(JEL).EQ.4.AND.ITNO.EQ.2) THEN
       GOTO 80
       ELSEIF (iNCN_MULTI_COMB(JEL).EQ.4.AND.ITNO.EQ.1) THEN
...
80    CONTINUE
      ...
      RETURN
      END SUBROUTINE SGLINTBD_MULTI_TRI2
...
!------------------------------------------------------------------------------- 
       ...
       SUBROUTINE SGLINTBD_MULTI_QUAD2(XF,JEL,RK,IRR)                                                  ! This is for quardilateral elements
       IMPLICIT   NONE
...
80    CONTINUE
      ...
      RETURN
      END SUBROUTINE SGLINTBD_MULTI_QUAD2

!-------------------------------------------------------------------------------
!         Calculate determinant of a matrix
!------------------------------------------------------------------------------- 

    SUBROUTINE BSDET_MULTI(A,N,DET)        ! The same function is implemented in SingularIntgr as well. This is just done as separation. 
    IMPLICIT NONE
...
100	CONTINUE
	DET=F*DET*A(N,N)
	RETURN
    END SUBROUTINE BSDET_MULTI

!-------------------------------------------------------------------------------
END MODULE SingularIntgrMulti
!*******************************************************************************