...
!   i) w*w/g = k tanh ( kh )
!   ii) -w*w/g = Um tan ( Umh ) - The first root is complex representing the pulsating wave with other roots representing evanescent modes of the eigenfunction expansion
!
...
!   if y=x, use the approximation directly instead of iteration
!   to avoid the singularity in the denominator of the transcendental
!   function; otherwise, do the iterative procedure. 
!
      X=W*W*H/G
      IF (X.GT.0.D0.AND.X.LE.2.D0) THEN                ! X.GT.0 means X greater than 0.0000 (till  (Reference: http://www.math.hawaii.edu/~hile/fortran/fort2.htm)
       Y=DSQRT(X)*(0.9994D0+0.1701D0*X+0.0305*X*X)     ! D0 is to double precision, for example 0.0D0 is 0.0 x 10 to the power 0. The number of significant digits after 0 is based on the declaration, in this case, REAL*8
      ELSE
       T=X*DEXP(-2.D0*X)
       Y=X+2.D0*T-6.D0*T*T
      ENDIF
      ...
      ELSE
       ...
       DO WHILE (EXX.GT.1.0D-10)
        ...
        M=M+1
       ENDDO
       WVN(1)=Y/H                                      ! The first root is obtained here
      ENDIF

!------------------------------------------------------------------
! II. Calculation of roots of Eq. (ii), which characterizes
!     the evanescent modes in eigenfucntion
!------------------------------------------------------------------
!
