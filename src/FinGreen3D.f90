...
!     
        ...
        IF (ABS(V).LT.1.E-8) THEN                                  ! This is for 0 incident wave frequency

          GOTO 100

        ELSEIF (ABS(V+1.D0).LT.1.E-8) THEN                         ! This is for infinite incident wave frequency
            
          GOTO 200

        ELSE
            
          CALL FINGREEN3D_R(RR,ZF,ZP,V,WVN,NK,H,FG,FLAG)
...
!!-------------------------------------------------------------------------------!!
      ...
      SUBROUTINE FINGREEN3D_R(RR,ZF,ZP,V,WVN,NK,H,GRN,TAG)

        IMPLICIT NONE
        INTEGER TAG,NK
        REAL*8 R,R1,RR,ZPQ,PI4,G,RHP,WK
        REAL*8,INTENT(IN)::ZF,ZP,V,WVN(1:NK),H
...
!     
        ...
        R =DSQRT(RR**2+(ZP-ZF)**2)                                               ! Distance between the source point and field point
        R1 =DSQRT(RR**2+ZPQ*ZPQ)                                                 ! Distance between the field point and the image of the source point w.r.t the free surface
        WK=WVN(1)                                                                ! Definition given at the beginning
        
! Choose an appropriate method to calculate the Green function
! based on the value of RHP (R/h)                                                                   ! This is the domain decomposition
!  
        RHP=DABS(RR/H)                                                           ! RR - Horizontal distance between the source point and the field point, H - Depth
...
! choose TAG=1 if you use higher-order boundary element method                                      ! Implementation of higher order boundary element method is also possible here
!  
        IF (TAG.EQ.1) THEN        
        CONTINUE
