...
MODULE PatcVelct
   ...
   USE HAMS_mod                                ! This is for the declaration of the required variables
   USE Const_mod 
   USE WaveDyn_mod
   USE PanelMesh_mod
...
!
        ...
        REAL*8,INTENT(IN):: X,Y,Z,XW,YW,SITA                           ! What is the value of SITA here? It is not assigned a value anywhere
        REAL*8:: WKX,DUM,W0,WK0
        
        IF (ABS(V).LT.1.E-8) THEN                                      ! V is the wavenumber at infinite depth = omega^2/g
         W0=1.E-20                                                     ! W0 is the frequency. In this case it is the first value, but in the else statement, it takes values greater than 0.
         WK0=1.E-20
        ...
        ELSE
         W0=W1
         WK0=WK                                                       ! In the else statement, WK represents the first root of the wave dispersion equation obtained using FINGREEN (the value k)
        ENDIF
 
        IF (Z.GT.0.D0) WRITE(6,*) 'Z>0, IN VINP', 'Z=',Z 
        CI=CMPLX(0.0D0,1.0D0)
 
        WKX=WK0*((X-XW)*COS(SITA)+(Y-YW)*SIN(SITA))                    ! SITA is mostly the beta value here, which is the inclination of the incident wave with the x-axis, but it is not given a value here

	    IF (H .LT. 0.0D0 .or. H .GT. 500.0D0) THEN                     ! This is to calculate the potential function, either with deep water or shallow water.
                                                                       ! Between 0 m and 500 m is considered shallow water here. 
                                                                       ! For shallow water, Equation 6 from 'HAMS: A Frequency-Domain Preprocessor forWave-Structure Interactions—Theory, Development,and Application' is used
                                                                       ! For deep water it is Equation 5 
          VINP=-CI*AMP*G/W0*EXP(WK0*Z)*EXP(CI*WKX)
	    ELSE
          DUM=-AMP*W0/(WK0*SINH(WK0*H))                                ! Shouldn't this be cosh based on Equation 6?
