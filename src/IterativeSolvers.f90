...
!
!    Preconditioners:
!      - Block-diagonal LU (default, set up by SETUP_BLOCK_PRECONDITIONER for
!        multi-body problems: one dense LU per body's self-interaction block).
!      - Jacobi/diagonal (fallback for single-body problems).
!
!    Uses inline BLAS operations (unrolled ZDOTC / DZNRM2 / ZGEMV) to avoid
!    known ifort/MKL calling-convention issues with the compiled BLAS bindings.
!
!    References:
...
MODULE IterativeSolvers
    ...
    ! Only two names are needed by AssbMatx*.f90:
    !   ZITER_SOLVE_MULTI            - single entry point for GMRES solves
    !   SETUP_BLOCK_PRECONDITIONER   - assemble block-diagonal LU preconditioner
    ! Everything else is module-private (ZGMRES_SOLVE, ZBLOCK_GMRES_SOLVE, the
    ! Jacobi setup, and the internal preconditioner state).
...
!
!   Structure is a plain serial loop over columns — each RHS is solved by a
!   separate ZGMRES_SOLVE call reusing the preconditioner. RHS-level OpenMP
!   parallelism was tested but caused L3 cache thrashing at N ~ 7000 because
!   several threads competed for the same H-matrix data; the serial version
!   keeps the working set in cache and is faster for the problem sizes we
!   target. The disabled OMP directives are left in place for future use with
!   much larger systems (N > ~15000) where the trade-off may flip.
!
!   The name "BLOCK" is historical — this is NOT a true Block-Krylov method
!   (no shared subspace, no ZGEMM matvec). Renaming would ripple through the
!   Iterative_solver branch history, so the identifier is kept.
!
!   Arguments:
!     N        - System size
!     AMAT     - Coefficient matrix (N x N, not modified)
...
!     INFO     - Output: 0 = all converged, non-zero = at least one did not
!---------------------------------------------------------------------------------------------
    ...
    SUBROUTINE ZBLOCK_GMRES_SOLVE(N, AMAT, B, X, NRHS, TOL, MAXITER, MRESTART, INFO)
        ...
        ! Fall back to the Jacobi (diagonal) preconditioner if no block-diagonal
        ! preconditioner has been set up (single-body path — SETUP_BLOCK_PRECONDITIONER
        ! is only called by the multi-body assemblers).
...
!---------------------------------------------------------------------------------------------
!   Multi-RHS entry point. Solves A*X = B for NRHS right-hand-side columns using
!   restarted GMRES with the currently installed preconditioner.
!
!   ISOLV_IN is accepted (and passed through unchanged) so that the caller's
!   dispatch value is preserved on the call stack for future extension to
!   additional iterative solvers. Presently only ISOLV=2 (GMRES) reaches this
!   routine — ISOLV=1 is handled directly by ZGETRS in the AssbMatx* callers.
!---------------------------------------------------------------------------------------------
