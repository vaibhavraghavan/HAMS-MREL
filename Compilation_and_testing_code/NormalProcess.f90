...
!   !   and on the inner water plane                                                                                     ! Inner water plane is for irregular frequency removal?


      SUBROUTINE CalRotNormals(RG,XYZ_P,DXYZ_P,NELEM)
      IMPLICIT   NONE
      
      INTEGER,INTENT(IN):: NELEM
      REAL*8,INTENT(IN):: RG(3),XYZ_P(NELEM,3)
	  ...
	  DO 20 IEL=1,NELEM
          ...
          DXYZ_P(IEL,4)=(XYZ_P(IEL,2)-RG(2))*DXYZ_P(IEL,3)-(XYZ_P(IEL,3)-RG(3))*DXYZ_P(IEL,2)
          DXYZ_P(IEL,5)=(XYZ_P(IEL,3)-RG(3))*DXYZ_P(IEL,1)-(XYZ_P(IEL,1)-RG(1))*DXYZ_P(IEL,3)
          DXYZ_P(IEL,6)=(XYZ_P(IEL,1)-RG(1))*DXYZ_P(IEL,2)-(XYZ_P(IEL,2)-RG(2))*DXYZ_P(IEL,1)
...
!   !   CalTransNormals is used to calculate normals on the immersed body surface                                      ! Which normals are these?
...
!     
	  DO IEL=1,NELEM
        ...
        IF (NCN(IEL).EQ.3) THEN                                                                            ! This is in the case of triangular elements. 3 nodes
              
         V21(:)=XYZ(NCON(IEL,1),:)-XYZ(NCON(IEL,2),:)                                              
         V23(:)=XYZ(NCON(IEL,3),:)-XYZ(NCON(IEL,2),:)

         UN(1)=V21(2)*V23(3)-V21(3)*V23(2)                                                                 ! This is the calculation of the cross product to determine the normal vector
         UN(2)=V21(3)*V23(1)-V21(1)*V23(3)
         UN(3)=V21(1)*V23(2)-V21(2)*V23(1)

         AUN=SQRT(UN(1)**2+UN(2)**2+UN(3)**2)

         DXYZ_P(IEL,1)=UN(1)/AUN                                                                           ! This is obtaining the unit normal vector
         DXYZ_P(IEL,2)=UN(2)/AUN
         DXYZ_P(IEL,3)=UN(3)/AUN
                   
        ELSEIF (NCN(IEL).EQ.4) THEN                                                                        ! This is in the case of quadrilateral elements. 4 nodes
...
!   !   CalPanelCentre is used to calculate the centers of the panels on the immersed body surface 
!   !   and on the inner water plane
...
! 
	 DO IEL=1,NELEM
        ...
        CALL CalDeltaArea_Improved(XYZ(NCON(IEL,1),:),XYZ(NCON(IEL,2),:),XYZ(NCON(IEL,3),:),DS(IEL))            ! Here the area improvements are done in two steps. When it is triangular elements, just the three coordinates are possible. However with a quadilateral, an additional node is present, hence another combination of nodes is used for improvement of the area prediction.
...
!=======================================================================
!SUBROUTINE CalPanelArea_Normal                                                                                      ! This sub-routine seems to be calculating the normal vector and area of the panels which is what the previous two functions do. This can be optimized by just calling the other two subroutines? 
...
!   !   on the immersed body surface and on the inner water plane
      ...
      SUBROUTINE CalPanelArea_Normal(RG,XYZ,NTND,NELEM,NCN,NCON,DS,XYZ_P,DXYZ_P)
      IMPLICIT   NONE

      INTEGER,INTENT(IN):: NTND,NELEM
      INTEGER,INTENT(IN):: NCN(NELEM),NCON(NELEM,4)
      REAL*8,INTENT(IN):: RG(3),XYZ(NTND,3),XYZ_P(NELEM,3)
      REAL*8,INTENT(OUT):: DS(NELEM),DXYZ_P(NELEM,6)
...
!=======================================================================
!SUBROUTINE CalDeltaArea                                                                                                             ! What is the use of this function?This is not really used in the code. There is an improved version which is utilized for a more accurate calculation.
...
! ========================================================
!     
      SUBROUTINE CalDeltaArea_Improved(P1,P2,P3,DS)                                                                                 ! An algorithm to obtain a more precise calculation of the area
      IMPLICIT   NONE
...
! ========================================================
!     
      SUBROUTINE CalPanelChartLength(XYZ,NTND,NELEM,NCN,NCON,PNSZ)                                                               ! This seems to be just obtaining the longest side of the panel. What is this used for?
      IMPLICIT   NONE
