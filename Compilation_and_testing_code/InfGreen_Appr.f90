...
SUBROUTINE INFGREEN3D(XF,XP,YF,YP,ZF,ZP,V,GRN,FLAG)
        ...
        XPQ=XF-XP                               ! XF is from the source point (the input is XQ when called from the CALGREEN subroutine), XP is from the field point.
        YPQ=YF-YP
        ZPQ=ZP+ZF
        
        RR  =SQRT(XPQ**2+YPQ**2)                ! Horizontal component of the distance R or R1
        R =SQRT(RR**2+(ZP-ZF)**2)               ! Distance between the source point and the field point
        R1 =SQRT(RR**2+ZPQ**2)                  ! Distance between the source point and the image of the field point w.r.t the mean free surface 
        
        GRN(:)=CMPLX(0.D0,0.D0)                 ! Initializing the Green's function
        
        CALL HavelockGF(V*RR,V*ZPQ,GF,GFH)      ! V is the wave number in deep water. It is used to make the coordinates dimensionless. Refer to Eqn 1 in Ref[1].This also computes the local and oscillatory terms of the green's function. 


        IF (ABS(V).LT.1.E-8) THEN               ! This is when dealing with very small wave numbers
...
! --- Local variables -------------------------------------
	...
	GF_alpha	=	-vv/GF_dd              ! Coming from Eqn 15 in Ref[1]
	GF_beta		=	hh/GF_dd               ! Coming from Eqn 15 in Ref[1]
	GF_sigma	=	hh/(GF_dd-vv)          ! Coming from Eqn 16 in Ref[1]
	GF_rho		=	GF_dd/(1.0D0+GF_dd)    ! Coming from Eqn 21a in Ref[1]
...
! --- Local variables -------------------------------------
	...
	GF_Func_W	=	2.0D0*pi*(H0-Im*J0)*dexp(vv)             ! This is the wave term of the function given by Eqn 7 in Ref[1]
...
! --- Local variables -------------------------------------
	...
	GF_FuncA	=	(A8+A9*tt)*tt                               ! This function gives the polynomial A(rho) which is used in approximating the local component L. Check from Eqn 33(d) in Ref[1]
...
! --- Local variables -------------------------------------
	...
	GF_FuncB	=	(B8+B9*tt)*tt                               ! This function gives the polynomial B(rho) which is used in approximating the local component L. Check from Eqn 33(e) in Ref[1]
...
! --- Local variables -------------------------------------
	...
	GF_FuncC	=	(C6+C7*tt)*tt                                     ! This function gives the polynomial C(rho) which is used in approximating the local component L. Check from Eqn 33(f) in Ref[1]
...
! --- Local variables -------------------------------------
	...
	GF_FuncD	=	(D8+D9*tt)*tt                                          ! This function gives the polynomial D(rho) which is used in approximating the local component L. Check from Eqn 33(g) in Ref[1]
