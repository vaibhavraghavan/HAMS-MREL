...
HAMS-MREL supports two linear solver options for the BEM system of equations. The solver is selected via the `ISOLV` parameter in [`ControlFile.in`](input_files.md).
...
| :--- | :--- | :--- | :--- |
| (optional) | `26x,i16` | `ISOLV` | Linear solver type: 1 = Direct LU, 2 = GMRES |
...
The three convergence-related tolerances in the iterative path are matched to the intrinsic precision of the assembled BEM system:

| Stage | Tolerance | Where |
|---|---|---|
| ACA compression of admissible H-matrix blocks | `ACA_TOL = 1e-4` | [`HMatrix.f90`](../src/HMatrix.f90) line 60 |
| GMRES residual `‖r‖ / ‖b‖` | `ITER_TOL = 1e-4` | [`IterativeSolvers.f90`](../src/IterativeSolvers.f90) line 50 |
| Effective per-entry precision of `AMAT` with `GRN_KIND=4` | ~ `1e-7` | Green's-function values in single precision (`WavDynMods.f90` line 39) |

For an admissible block of size `NR × NC` with per-entry noise `1e-7`, the noise contribution to the Frobenius norm is roughly `sqrt(NR × NC) × 1e-7 × ‖block‖`. Setting this equal to `ACA_TOL × ‖block‖` gives a block-size safety limit of about 1000 × 1000. Cluster leaves of 32 keep admissible blocks well below this, so ACA reaches its target tolerance cleanly for typical mesh sizes.

**Practical consequence:** running `GRN_KIND=4` together with `ISOLV=2` produces hydrodynamic coefficients that agree with the `GRN_KIND=8 + ISOLV=1` reference to 4–5 significant figures on physically meaningful entries. Sub-noise-floor terms (cross-DOF couplings that should be zero) may show larger relative differences but are physically zero. For maximum accuracy against WAMIT references, use `GRN_KIND=8 + ISOLV=1`.

If tighter accuracy is needed, both `ACA_TOL` and `ITER_TOL` are `PUBLIC` module parameters and can be lowered at runtime (or edited before compilation). Do not push them below `1e-6` when `GRN_KIND=4` — the noise floor swamps the target.
...
3. **Check mesh quality**: Poorly shaped panels (very elongated or with near-zero area) degrade matrix conditioning.
4. **Use `GRN_KIND=8`**: Full-precision Green's-function assembly gives GMRES more headroom to reach tighter residuals if needed.
