...
ifx -qmkl -heap-arrays 0 -o HAMS_MREL WavDynMods.f90 InputFiles.f90 WavDynSubs.f90 PatclVelct.f90 FinGrnExtSubs.f90 InfGreen_Appr.f90 FinGreen3D.f90 SingularIntgr.f90 SingularIntgrMulti.f90 CalGreenFunc.f90 CalGreenFuncMulti.f90 BodyIntgr_irr.f90 BodyIntgr.f90 BodyIntgr_irrMulti.f90 BodyIntgrMulti.f90 HMatrix.f90 IterativeSolvers.f90 AssbMatx_irr.f90 AssbMatx.f90 AssbMatx_irrMulti.f90 AssbMatxMulti.f90 HydroStatic.f90 HydroStaticMulti.f90 NormalProcess.f90 ReadPanelMesh.f90 ReadPanelMeshMulti.f90 ImplementSubs.f90 PotentWavForce.f90 PotentWavForceMulti.f90 PressureElevation.f90 PressureElevationMulti.f90 SolveMotion.f90 SolveMotionMulti.f90 PrintOutput.f90 HAMS_Prog.f90 -qopenmp

# Run the appplication
./hamsmrel <path-to-input-files> <path-to-output-directory>
```

This will create an executable called `hamsmrel` (specified by the `-o` option) inside the HAMS-MREL/src directory. The flags above mean:

| Flag | Purpose |
|---|---|
| `-qmkl` | Link Intel MKL (multi-threaded `ZGETRF` / `ZGETRS`) |
| `-qopenmp` | Enable OpenMP parallelisation |
| `-heap-arrays 0` | **Required.** Forces all automatic arrays and compiler-generated array temporaries onto the heap. Without it, multi-body cases with TNELEM ≳ 3000 abort with `forrtl: severe (170): Program Exception - stack overflow` on the first frequency iteration — the default thread stack is too small for the array-section temporaries created inside the deeply-inlined `CALGREEN_*` / `SGLINTBD_*` call chain. The `0` argument means "no size threshold". On HPC clusters you can additionally raise the stack ceiling at job-script level with `ulimit -s unlimited`. |
...
export FPM_FFLAGS="-qmkl -qopenmp -heap-arrays 0"       # flags — `-heap-arrays 0` is required, see the
                                                        # note in the manual-ifx section above.
...
This produces the executable `hamsmrel` in the `src` directory. The `-qmkl` and `-qopenmp` flags link the MKL library and enable OpenMP parallelization. The `-heap-arrays 0` flag forces all automatic arrays onto the heap — without it, large multi-body cases overflow the per-thread stack and abort on the first frequency. See the explanation in the [non-HPC build section above](#step-3-build-and-execute) for details.
...
# Lift the per-thread stack limit. Although the binary should have been compiled with `-heap-arrays 0`,
# raising the soft stack limit here removes one more failure mode for very large multi-body runs.
ulimit -s unlimited
export OMP_STACKSIZE=512M     # generous per-thread stack for the OpenMP workers
