"""
The functions in this script can be directly utilized by users to just retrieve the data regarding the hydrodynamic
coefficients and exciting forces from the output of HAMS-MREL for multiple bodies
"""
...
def get_mass_and_damping_matrices_hams_multibodies(added_mass_file_location: str, added_damping_file_location: str, dof: int):
    """
    This function obtains the added mass and radiation damping coefficients for multiple bodies from the HAMS-MREL output
    """
    # Mass matrix
    # Importing the files
...
def get_external_force_hams_multibodies(external_force_file_location: str, column: int):
    """
    This function obtains the excitation force for multiple bodies from the HAMS-MREL output
    """
    # Importing the files
...
def compile_mass_damping_matrices_multiple_bodies(n_bodies: int, length_lists: int, added_mass_final: list,
    ...
    """
    This function creates the compiled list of added mass and radiation damping coefficients for multiple bodies from the HAMS-MREL output
    """
    added_mass_compiled = []
...
def compile_excitation_force_matrix_multiple_bodies(n_bodies: int, length_lists: int, exciting_force_final: list, n_directions: int):

    exciting_force_compiled = []

    for j in range(n_bodies):
        for l in range(n_directions):
            exciting_force_compiled.append([exciting_force_final[n_bodies * n_directions * k + l*n_bodies + j] for k in range(length_lists)])

    # The resulting list exciting_force_compiled is of the format (n_bodiesxn_direction)xomega. This means for primary list is of the size (n_bodiesxn_direction) with the first list belonging to body 1 direction 1 and the second list belonging to body 1 direction 2
...
def hydrodynamic_coefficients_exciting_forces_all_bodies(added_damping_file_location: str,
    ...
    """
    This function creates the compiled list of hydrodynamic coefficients and exciting forces for multiple bodies
    from the HAMS-MREL output for all frequencies
    """
    ...
    omega_final = remove_duplicates(omega)
    n_bodies = int(math.sqrt(len(omega) / len(omega_final))) # No of bodies - This formulation works irrespective of the number of directions because these are taken from the added mass/radiation damping files, that are independent of direction
    n_directions = int(len(external_force_all_frequencies_1)/(len(omega_final)*n_bodies))
    omega_final.sort()
    ...
    return n_bodies, omega_final, added_mass_compiled, radiation_damping_compiled, exciting_force_compiled, n_directions
...
def hydrodynamic_coefficients_exciting_forces_all_bodies_zero_inf(added_damping_file_location: str,
    ...
    """
    This function creates the compiled list of hydrodynamic coefficients and exciting forces for multiple bodies
    from the HAMS-MREL output for only zero and infinite frequency
    """
    ...
    return n_bodies, omega_final, added_mass_compiled, radiation_damping_compiled

def obtain_specific_coefficient_matrix(added_mass_compiled: list, radiation_damping_compiled: list, n_bodies: int,
                                       x: int, y: int):
    """
    This function can be used to retrieve a specific added mass and radiation damping coefficient list depending on the
    degree of freedom indicated
    """
    index = n_bodies * (x - 1) + (y - 1)
    return added_mass_compiled[index], radiation_damping_compiled[index]


def obtain_specific_exciting_force_matrix(exciting_force_compiled: list, x: int, j:int = 1, n_directions: int = 1):
    """
    This function can be used to retrieve a specific excitation force list depending on the
    degree of freedom indicated
    i is for the body number and j is for the direction number
    By default the number of directions is 1
    """
    return exciting_force_compiled[n_directions*(x-1) + (j - 1)]